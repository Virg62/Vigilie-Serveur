...
{
    global $db;
    $rep = ["success" => false, "message" => "impossible de modifier le profil.", "data" => null];
    try {
        if ($key === "password") {
            ...
            if (in_array($key, $available_choices)) {
                ...
                $rep["success"] = true;
                $rep["message"] = "La valeur à bien été changée";
            } else {
...
{
    global $db;
    $rep = ["success" => false, "message" => "echec de la récupération du profil", "data" => null];
    try {
        ...
        $rep["success"] = true;
        $rep["message"] = "Le profil a été récupéré avec succès !";
        $rep["data"] = $profile;
